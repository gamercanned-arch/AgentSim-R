...
API_CONTEXT_SIZE = int(os.environ.get("API_CONTEXT_SIZE", "1000000"))
...
def _load_models_list(filepath: str = "models_list") -> list[str]:
    """Loads models from the models_list file, returning a safe default if missing."""
    try:
...
def _models_from_env(provider_prefix: str, default: str) -> list[str]:
    ...
    if not models:
        # Check for _MODEL_1 through _MODEL_N dynamically
        i = 1
        misses = 0
        while misses < 20:
...
def main():
    ...
    if save_exists(SAVE_PATH):
        ...
        _wipe_cache_and_logs()
        world = build_starting_world()
# Automatically set to ordered Gemini
    ...
    # Load defaults from models_list
    default_models = _load_models_list()
    
    # If the user didn't specify GEMINI_MODELS in .env, it uses models_list defaults
    ...
    print(f"\nAgentSim-R API runner starting using Google Gemini...\nTime limit: {MAX_RUNTIME_MINUTES}m")
    tick = 0
