...
def _assistant_history_text(out: str) -> str:
    cleaned = _THINK_BLOCK_RE.sub("", str(out or "")).strip()
    return cleaned
...
def run_tick(world) -> bool:
    ...
    if estimated_tokens + MAX_NEW_TOKENS >= context_limit:
        agent.last_action_result = "Prompt still exceeds context limit after trimming."
    ...
    gen, prompt_tokens, gen_tokens = call_server(
    ...
    parsed_calls, parse_error = parse_tool_calls(gen)
    if parse_error:
...
def _apply_passive_updates(agent, world, current_time: float) -> None:
    ...
    if agent.health <= 0.0:
        from python.tools import _kill_agent
...
def _attempt_emergency_consume(agent, world, mode: str = "hunger") -> None:
    ...
    for i, item in enumerate(agent.inventory):
        name = item["item"]
        if name not in ITEM_CATALOG["food"]:
            continue
