...
Agents are instructed to output **exactly one** XML tool call per turn.
...
Important parser/runtime notes:
- The parser is now **strict** about stray text.
- Only an optional leading `<think>...</think>` block is tolerated outside tool XML.
- Extra text before, between, or after tool calls is treated as a parse failure.
- Runtime validates required and unexpected parameters against tool schemas.
...
Per-agent history is trimmed when prompts grow too large. The engine tries to preserve recent structured turns and continue the simulation rather than stopping because one agent accumulated too much history.
...
- `python/bootstrap.py`: shared starting-world initialization

Tool engine:
- `python/tooling/execute.py`: dispatcher + registry + runtime schema validation
...
Homes are treated as private locked spaces unless the agent owns that home.
...
- if fuel is unaffordable, movement fails (no walk fallback)
...
The prompt surfaces a compact but actionable subset including:
- time/date
- location + "inside/outside"
- needs + money
- education, relationship status, partner, beliefs/goals
- full inventory summary + held item
- nearby people + visible objects
- nearby ground items + nearby corpse estates
- nearby open/closed public locations
- active task instructions (if any)
- voicemail summary
...
$$
\text{market}_{\text{anxiety}} \propto w_3 \cdot |\Delta P| \cdot \frac{\text{position}_{\text{value}}}{\max(0, \text{money}) + 1} \quad \text{where } w_3 = 0.5
$$

Base stress target:
$$
\Psi^* = \frac{\text{rel}_{\text{tension}} + \text{fin}_{\text{pressure}} + \text{market}_{\text{anxiety}}}{1 + \alpha \cdot \text{happiness} + \beta \cdot \text{hourly}_{\text{wage}}}
$$
...
Update:
$$
\text{stress}_{t+1} = \mathrm{clamp}_{[0,100]}\left(0.7 \cdot \text{stress}_t + 0.3 \cdot \Psi^* \cdot \text{debt}_{\text{penalty}}\right)
$$
...
Health delta:
$$
\Delta\text{health} = \Bigl[-\bigl(0.5 \cdot \text{stress} + 0.3 \cdot \text{hunger} + 10 \cdot \text{energy}_{\text{penalty}} + D\bigr) \cdot 0.1 \cdot \text{happiness}\Bigr] \cdot A \cdot 0.02
$$
...
Emergency triggers:
- if awake and hunger ≥ 90 → attempt emergency consume/buy using actual food that reduces hunger
...
- must remain in the correct location while completing the task
- task steps are locked: unrelated tools are blocked mid-task
- task prop/target only count while active and visible in the valid task location
...
$$
\text{pay} = \text{hourly}_{\text{wage}} \cdot \text{hours} \cdot \frac{\text{market}_{\text{price}}}{100}
$$
...
Price updates every 5 minutes during open hours (`MARKET_TICK_SECONDS = 300`):
$$
...
- `talk_to`: requires proximity (50m). If target is busy/sleeping → initiator fails but target receives a **missed interaction** notification.
- `give_item`: requires proximity (20m). If target is busy/sleeping → transfer is **queued** and escrowed immediately, then delivered once the target becomes available.
- If queued item delivery arrives but recipient inventory is full → delivery cancels and returns to sender (or sender estate / nearby world preservation edge case).

Messages passed through social tools are sanitized/truncated before being injected into prompts or logs.
...
Notifications shown in a prompt are only consumed after a successful action resolution.
...
- structured `Role: {System, User, Agent, Tool}` fields
...
Default initialization is centralized in `python/bootstrap.py` and used by both:
- `python/sim.py`
- `extract_t1.py`
